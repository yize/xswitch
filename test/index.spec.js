...
describe('CORS without Access-Control-Allow-Origin', () => {
  test('should support cors', () => {
    window.proxyConfig.proxy = [
        ['http://dev-smart.youku.com/(.*).json','http://pre-youku-smart.youku.com/$1.json'
      ]
    ];
    ...
    expect(window.onHeadersReceivedCallback(testheaderDetails).responseHeaders).toEqual(expectHeaderDetails);
  });
});
  ...
  test('should support cors', () => {
    window.proxyConfig.proxy = [
      [
        ...
        "http://ottscg.alibaba.net/$1.json"
      ]
    ];
    var testheaderDetails = {
      "frameId":0,
...
describe('CORS withCredentials and no proxyConfig', () => {
  test('should return {}', () => {
    window.proxyConfig.proxy = [];
    expect(window.onHeadersReceivedCallback({})).toEqual({});
  });
});
